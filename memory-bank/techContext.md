...
- After deployment validation, one exact `ReleaseCompletion` approval may cover one 360-second application smoke and, only after it succeeds, one separate fresh browser demo below 240 seconds. The smoke ceiling accommodates the bounded 90-second campaign plus measured reproduction, minimization, and verify-fix replay without weakening the one-run rule. `deploy/scripts/release-completion.ps1` preserves distinct smoke/demo evidence and durable hunt/run/finding IDs; interruption may resume only that same application run, while an ambiguous ID-less mutation forbids a second new run. This consolidation grants no Terraform, image, IAM/API, token, secret-read, cleanup, deletion, or destruction authority.
- A deterministic campaign attempt whose invariant outcome is `Fail` ends adaptive strategy selection immediately after the attempt checkpoint is durable. The loop returns `VerifiedViolation` without another model call or agent-decision event, preserving the remaining physical-request budget for the required reproduction, minimization, and verify-fix evidence.

...
**Notes**: Foundation, application deployment, topology validation, and external route validation are complete and must not be repeated. Preserve the immutable API and reference-target images after exact input-equivalence proof. The worker source changed and requires a new commit-bound immutable worker image. Resume only from the consolidated exact `PublishWorkerAndPlan` gate: one token, worker-only immutable publication/digest resolution, exact tfvars generation, and one saved Terraform plan plus sanitized review. It authorizes no apply, validation, smoke, demo, cleanup, destruction, other image operation, IAM/API change, or credential creation.

...
**Notes**: Foundation, immutable image publication, application Terraform apply, topology validation, and external route validation are complete and must not be repeated. Resume only from an exact no-token `ApplicationCompletionResume` gate bound to hunt `bef0d703-ceaf-4112-a257-58f537ff092d` and plan `plan-472b197b63f92131`. It may reset the expired application deadline, create the run/replay for that existing hunt, and after successful smoke record one distinct demo plus credential-free evidence/task completion. It forbids POSTing a hunt or plan, Terraform, credentials, control-plane calls, cleanup, destruction, or a second new smoke/demo after ambiguity.

...
**Completed**: —
**Output**: Deployment, topology, and external route validation succeeded. The preserved application run `85fc22fd-43a3-4e9f-88a6-13659d617645` failed in the worker before producing target trace evidence. The exact narrow log diagnostic returned zero real entries after the response-shape defect was corrected. A wider, revision-and-time-bound read-only Logging diagnostic is the next mandatory approval gate; no second run or demo is authorized.
...
**Notes**: Foundation, immutable image publication, application deployment, topology validation, and external route validation are complete and must not be repeated. Preserve hunt `b1202777-77b1-411a-9b0d-10e49f5cfa2e`, plan `plan-91944ef4a61be989`, and failed run `85fc22fd-43a3-4e9f-88a6-13659d617645`; never create a second run under a diagnostic gate. Resume only from the exact `WorkerFailureWindowDiagnostic` approval prepared for one sanitized, non-paginated Cloud Logging read of worker revision `racehunter-worker-00001-twd` in the fixed three-minute window. Do not run Terraform, republish images, retry the campaign, or begin demo.

...
test('local admin can configure and replay the controlled HTTP JSON target', async ({ page }) => {
  ...
  await expect(page.getByLabel('Authorized host')).toHaveValue('reference-target')
  await expect(page.getByLabel('Authorized port')).toHaveValue('8080')
  await expect(page.getByLabel('Setup request JSON template')).toHaveValue(/vulnerable/)
  await expect(page.getByLabel('Operation request JSON template')).toHaveValue(/\{\{actorId\}\}/)
  await expect(page.getByLabel('Primary observation JSON path')).toHaveValue('$.reservationCount')
  await expect(page.getByLabel('Invariant family')).toHaveValue('numeric-boundary')
  await expect(page.getByLabel('Sensitive JSON paths')).toHaveValue(/\$\.authorization/)
  await expect(page.getByLabel('Rendered request preview')).toContainText('actor-1')
